...
### Fixed
- **Login message form: user search dropdown was covered by the Save/Cancel buttons.** The autocomplete results list on `/admin/login-messages/new` (when `targetType=USERS`) appeared behind the form action buttons because its `position: relative` container did not create its own stacking context. Added `isolation: isolate` on `.autocomplete`, raised the dropdown z-index, and made the `no results` / loading hint use the same absolute positioning as the results list so it no longer pushes the buttons down. (`login-message-form.scss`)
